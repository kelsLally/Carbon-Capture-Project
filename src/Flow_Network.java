...
public class Flow_Network {
    ...
    /**
     * Uses Bellman-Ford to find the s-t path that would be cheapest to route amount units of flow along
     *
     * @param amount the maximum amount of flow desired
     * @return A tuple containing the path that was found, its cost, and its
     * maximum possible flow
     */
     ...
     * maximum possible flow
     */
    public PathCostFlow findCheapestPathNonNegativeCosts(double amount) {
        double[] cost = new double[n];
        int[] pred = new int[n];
        for (int i = 0; i < n; i++) {
            cost[i] = Double.MAX_VALUE;
        ...
        for (int i = 0; i < n - 1; i++) {
            for (Edge[] row : matrix) {
                for (Edge e : row) {
                    if (e != null && cost[e.getEnd()] > cost[e.getStart()] + e.getCostNonNegative(amount)) {
                        cost[e.getEnd()] = cost[e.getStart()] + e.getCostNonNegative(amount);
                        pred[e.getEnd()] = e.getStart();
                    }
                }
            }
            ...
            for (Edge e : row) {
                if (e != null && cost[e.getEnd()] > cost[e.getStart()] + e.getCostNonNegative(amount)) {
                    ...
                    for (int i = 0; i < cycle.size()-1; i++){
                        costs.add(matrix[cycle.get(i)][cycle.get(i+1)].getCostNonNegative(amount));
                    }
                    costs.add(matrix[cycle.get(cycle.size()-1)][cycle.get(0)].getCostNonNegative(amount));
     ...
     * @return false if the graph's max flow is less than demand, true o.w
     */
    public boolean solveSeanHeuristicNonNegative(double demand) {
        while (demand - getFlow() > 0) {
